...
const HAS_FETCH_CACHE = (IS_FETCH_ENV &&
...
const isIterable = (o) => isDefined(o) && typeof o[Symbol.iterator] === 'function';
...
const getComponentsDtsSrcFilePath = (config) => config.sys.path.join(config.srcDir, GENERATED_DTS);
const getComponentsDtsTypesFilePath = (config, outputTarget) => config.sys.path.join(outputTarget.typesDir, GENERATED_DTS);
...
const emptyDir = async (config, compilerCtx, buildCtx, dir) => {
...
const STENCIL_INTERNAL_PLATFORM_ID = '@stencil/core/internal/platform';
const STENCIL_INTERNAL_RUNTIME_ID = '@stencil/core/internal/runtime';
...
const appendBuildConditionals = (config, build, s) => {
    const builData = Object.keys(build).sort().map(key => (key + ': ' + (build[key] ? 'true' : 'false'))).join(', ');
...
const getContextImport = (platform) => {
    return `import { Context } from '${platform === 'hydrate' ?
...
const COMPILER_BUILD = {
    id: '20200127214506',
    minfyJs: 'terser4.6.3_5',
    optimizeCss: 'autoprefixer9.7.3_postcss7.0.26_5',
    transpiler: 'typescript3.7.5_5'
};
...
const createStencilImportPath = (tagName, encapsulation, modeName, importeePath, importerPath) => {
    const data = {
        tag: tagName,
    };
    ...
    let p = importeePath;
    if (path$1.isAbsolute(importeePath)) {
        const importerDir = path$1.dirname(importerPath);
        p = path$1.relative(importerDir, importeePath);
    }
    if (!p.startsWith('.')) {
        p = './' + p;
    }
...
const isCoreResource = (url) => CORE_RESOURCES.some(u => url.includes(u));
...
const writeFetchSuccessAsync = async (inMemoryFs, url, filePath, content, pkgVersions) => {
...
const fetchModuleAsync = async (inMemoryFs, pkgVersions, url, filePath) => {
...
const getStencilInternalModule = (rootDir, compilerExe, internalModule) => {
    if (isExternalUrl(compilerExe)) {
        return normalizePath(path$1.join(rootDir, 'node_modules', '@stencil', 'core', 'internal', internalModule, 'index.mjs'));
    }
    const compilerExeDir = path$1.dirname(compilerExe);
    return normalizePath(path$1.join(compilerExeDir, '..', 'internal', internalModule, 'index.mjs'));
};
...
const imagePlugin = (config, _compilerCtx, buildCtx) => {
    return {
        name: 'image',
...
async function getClientPolyfill(config, compilerCtx, polyfillFile) {
    const polyfillFilePath = config.sys.path.join(config.sys.compiler.packageDir, 'internal', 'client', 'polyfills', polyfillFile);
    return compilerCtx.fs.readFile(polyfillFilePath);
}
async function getAppBrowserCorePolyfills(config, compilerCtx) {
    ...
    const results = await Promise.all(polyfills
...
async function getSystemLoader(config, compilerCtx, corePath, includePolyfills) {
...
const writeCollectionManifests = async (config, compilerCtx, buildCtx, outputTargets) => {
    ...
    return Promise.all(outputTargets.map(o => writeCollectionManifest(config, compilerCtx, collectionData, o)));
};
...
const writeCollectionManifest = async (config, compilerCtx, collectionData, outputTarget) => {
...
const bundleHydrateFactory = async (config, compilerCtx, buildCtx, build, appFactoryEntryCode) => {
    try {
...
async function generateHydrateApp(config, compilerCtx, buildCtx, outputTargets) {
    try {
        const rollupOptions = Object.assign(Object.assign({}, config.rollupConfig.inputOptions), { input: config.sys.path.join(config.sys.compiler.packageDir, 'internal', 'hydrate', 'runner.mjs'), inlineDynamicImports: true, plugins: [
                {
...
async function generateHydrateFactoryEntry(config, compilerCtx, buildCtx) {
    const cmps = buildCtx.components;
...
const updateStencilTypesImports = (path, typesDir, dtsFilePath, dtsContent) => {
 ...
 */
const updateReferenceTypeImports = (config, importDataObj, allTypes, cmp, filePath) => {
...
const updateImportReferenceFactory = (config, allTypes, filePath) => {
...
const generateHostConfig = (config, compilerCtx, outputTarget) => {
    const buildDir = getAbsoluteBuildDir(config, outputTarget);
...
const generateUsages = async (config, compilerCtx, usagesDir) => {
    const rtn = {};
    try {
...
const depsToMarkdown = (config, cmp, cmps) => {
    const content = [];
...
const getCmpLink = (config, from, to, cmps) => {
...
const generateMarkdown = (config, userContent, cmp, cmps, footer) => {
    return [
        userContent,
...
async function optimizeCss(config, compilerCtx, diagnostics, styleText, filePath) {
...
const hasChangedImportFile$1 = async (config, compilerCtx, buildCtx, filePath, noLoop) => {
...
const hasChangedImportContent$1 = async (config, compilerCtx, buildCtx, filePath, content, checkedFiles) => {
    const cssImports = getCssImports(config, buildCtx, filePath, content);
    if (cssImports.length === 0) {
        // don't bother
        return false;
...
const parseImportLegacy = (config, compilerCtx, buildCtx, moduleFile, dirPath, importNode) => {
...
const parseStaticComponentMeta = (config, compilerCtx, typeChecker, cmpNode, moduleFile, nodeMap, transformOpts) => {
...
const addExternalImportLegacy = (config, compilerCtx, buildCtx, moduleFile, resolveFromDir, moduleId) => {
...
const styleToStatic = (config, newMembers, componentOptions) => {
    ...
    if (typeof componentOptions.styles === 'string') {
        ...
        if (convertIdentifier.__identifier) {
            ...
            //   }
            // })
            const styleModeIdentifiers = {};
            Object.keys(convertIdentifier).forEach(modeName => {
                const styleModeIdentifier = convertIdentifier[modeName];
...
const propDecoratorsToStatic = (config, diagnostics, decoratedProps, typeChecker, watchable, newMembers) => {
    const connect = [];
    const context = [];
...
const parsePropDecorator = (config, diagnostics, typeChecker, prop, context, connect, watchable, newMembers) => {
...
const validateManifestJsonData = async (config, compilerCtx, buildCtx, manifestFilePath, manifestData) => {
...
const validateManifestJsonIcon = async (config, compilerCtx, buildCtx, manifestFilePath, manifestIcon) => {
...
class Compiler {
    ...
    on(eventName, cb) {
        return this.ctx.events.on(eventName, cb);
    }
    once(eventName) {
        return new Promise(resolve => {
            const off = this.ctx.events.on(eventName, (...args) => {
                off();
...
const transpileModule = (config, input, transformOpts, sourceFilePath) => {
    ...
    const compilerCtx = new CompilerContext$1();
    const buildCtx = new BuildContext(config, compilerCtx);
