/*
 Stencil Runtime v1.9.0-14 | MIT Licensed | https://stenciljs.com
 */
'use strict';

var platform = require('@stencil/core/internal/platform');
var appData = require('@stencil/core/internal/app-data');

const addEventListeners = (elm, hostRef, listeners) => {
    hostRef.$queuedListeners$ = hostRef.$queuedListeners$ || [];
    const removeFns = listeners.map(([flags, name, method]) => {
        const target = (appData.BUILD.hostListenerTarget ? getHostListenerTarget(elm, flags) : elm);
...
const getHostListenerTarget = (elm, flags) => {
    if (appData.BUILD.hostListenerTargetDocument && flags & 4 /* TargetDocument */)
        return platform.doc;
    if (appData.BUILD.hostListenerTargetWindow && flags & 8 /* TargetWindow */)
        return platform.win;
    if (appData.BUILD.hostListenerTargetBody && flags & 32 /* TargetBody */)
        return platform.doc.body;
    if (appData.BUILD.hostListenerTargetParent && flags & 16 /* TargetParent */)
...
const installDevTools = () => {
    if (appData.BUILD.devTools) {
        const stencil = platform.win.stencil = platform.win.stencil || {};
...
const addStyle = (styleContainerNode, cmpMeta, mode, hostElm) => {
    let scopeId = appData.BUILD.mode ? getScopeId(cmpMeta.$tagName$, mode) : getScopeId(cmpMeta.$tagName$);
    let style = platform.styles.get(scopeId);
    ...
    if (style) {
        if (typeof style === 'string') {
            ...
            if (!appliedStyles.has(scopeId)) {
                if (appData.BUILD.hydrateClientSide && styleContainerNode.host && (styleElm = styleContainerNode.firstElementChild) && styleElm.tagName === 'STYLE') {
                    // This is only happening on native shadow-dom, do not needs CSS var shim
                    styleElm.innerHTML = style;
                }
                else {
                    if (appData.BUILD.cssVarShim && platform.plt.$cssShim$) {
                    ...
                    if (appData.BUILD.hydrateServerSide || appData.BUILD.hotModuleReplacement) {
        ...
        else if (appData.BUILD.constructableCSS && !styleContainerNode.adoptedStyleSheets.includes(style)) {
...
const attachStyles = (hostRef) => {
    ...
    const scopeId = addStyle((appData.BUILD.shadowDom && platform.supportsShadowDom && elm.shadowRoot)
        ? elm.shadowRoot
        : elm.getRootNode(), cmpMeta, hostRef.$modeName$, elm);
    if ((appData.BUILD.shadowDom || appData.BUILD.scoped) && appData.BUILD.cssAnnotations && flags & 10 /* needsScopedEncapsulation */) {
        ...
        if (appData.BUILD.scoped && flags & 2 /* scopedCssEncapsulation */) {
            elm.classList.add(scopeId + '-s');
        }
    }
    endAttachStyles();
};
const getScopeId = (tagName, mode) => 'sc-' + ((appData.BUILD.mode && mode) ? tagName + '-' + mode : tagName);
    ...
    const walk = (c) => {
        for (let i = 0; i < c.length; i++) {
            ...
            if (Array.isArray(child)) {
                ...
                else if (appData.BUILD.isDev && typeof nodeName !== 'function' && child.$flags$ === undefined) {
...
const parsePropertyValue = (propValue, propType) => {
    ...
    if (propValue != null && !isComplexType(propValue)) {
        if (appData.BUILD.propBoolean && propType & 4 /* Boolean */) {
            // per the HTML spec, any string value means it is a boolean true value
            // but we'll cheat here and say that the string "false" is the boolean false
            return (propValue === 'false' ? false : propValue === '' || !!propValue);
        }
        if (appData.BUILD.propNumber && propType & 2 /* Number */) {
            // force it to be a number
            return parseFloat(propValue);
        }
        if (appData.BUILD.propString && propType & 1 /* String */) {
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    ...
    else if (appData.BUILD.vdomListener &&
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (appData.BUILD.vdomText && newVNode.$text$ !== null) {
    ...
    else if (appData.BUILD.slotRelocation && newVNode.$flags$ & 1 /* isSlotReference */) {
        // create a slot reference node
        elm = newVNode.$elm$ = (appData.BUILD.isDebug || appData.BUILD.hydrateServerSide) ? platform.doc.createComment(`slot-reference:${hostTagName.toLowerCase()}`) : platform.doc.createTextNode('');
    }
    else {
        ...
        elm = newVNode.$elm$ = ((appData.BUILD.svg)
            ? platform.doc.createElementNS(isSvgMode ? SVG_NS : HTML_NS, (appData.BUILD.slotRelocation && newVNode.$flags$ & 2 /* isSlotFallback */) ? 'slot-fb' : newVNode.$tag$)
            : platform.doc.createElement((appData.BUILD.slotRelocation && newVNode.$flags$ & 2 /* isSlotFallback */) ? 'slot-fb' : newVNode.$tag$));
        ...
        if ((appData.BUILD.shadowDom || appData.BUILD.scoped) && isDef(scopeId) && elm['s-si'] !== scopeId) {
...
const addVnodes = (parentElm, before, parentVNode, vnodes, startIdx, endIdx) => {
    let containerElm = ((appData.BUILD.slotRelocation && parentElm['s-cr'] && parentElm['s-cr'].parentNode) || parentElm);
    let childNode;
    if (appData.BUILD.shadowDom && containerElm.shadowRoot && containerElm.tagName === hostTagName) {
    ...
    for (; startIdx <= endIdx; ++startIdx) {
        if (vnodes[startIdx]) {
            ...
            if (childNode) {
                ...
                containerElm.insertBefore(childNode, appData.BUILD.slotRelocation ? referenceNode(before) : before);
            }
        }
    }
};
...
const updateChildren = (parentElm, oldCh, newVNode, newCh) => {
    ...
    while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (oldStartVnode == null) {
            ...
            // Vnode moved right
            if (appData.BUILD.slotRelocation && (oldStartVnode.$tag$ === 'slot' || newEndVnode.$tag$ === 'slot')) {
...
const isSameVnode = (vnode1, vnode2) => {
    ...
    if (vnode1.$tag$ === vnode2.$tag$) {
        if (appData.BUILD.slotRelocation && vnode1.$tag$ === 'slot') {
...
const patch = (oldVNode, newVNode) => {
    ...
    if (!appData.BUILD.vdomText || newVNode.$text$ === null) {
        ...
        if (appData.BUILD.vdomAttribute || appData.BUILD.reflect) {
            if (appData.BUILD.slot && newVNode.$tag$ === 'slot') ;
            else {
        ...
        if (appData.BUILD.updatable && oldChildren !== null && newChildren !== null) {
            // looks like there's child vnodes for both the old and new vnodes
            updateChildren(elm, oldChildren, newVNode, newChildren);
        }
        else if (newChildren !== null) {
            // no old child vnodes, but there are new child vnodes to add
            if (appData.BUILD.updatable && appData.BUILD.vdomText && oldVNode.$text$ !== null) {
    ...
    else if (appData.BUILD.vdomText && appData.BUILD.slotRelocation && (defaultHolder = elm['s-cr'])) {
        // this element has slotted content
        defaultHolder.parentNode.textContent = newVNode.$text$;
    }
    else if (appData.BUILD.vdomText && oldVNode.$text$ !== newVNode.$text$) {
        // update the text content for the text only vnode
        // and also only if the text is different than before
        elm.data = newVNode.$text$;
    }
    if (appData.BUILD.svg && isSvgMode && newVNode.$tag$ === 'svg') {
        isSvgMode = false;
    }
};
...
const renderVdom = (hostRef, renderFnResults) => {
    ...
    // <Host> runtime check
    if (appData.BUILD.isDev && Array.isArray(renderFnResults) && renderFnResults.some(isHost)) {
...
render() {
  ...
  `);
    ...
    rootVnode.$elm$ = oldVNode.$elm$ = (appData.BUILD.shadowDom ? hostElm.shadowRoot || hostElm : hostElm);
        ...
        if (checkSlotRelocate) {
            ...
            for (let i = 0; i < relocateNodes.length; i++) {
                ...
                if (!relocateNode.$nodeToRelocate$['s-ol']) {
                    ...
                    const orgLocationNode = (appData.BUILD.isDebug || appData.BUILD.hydrateServerSide)
                        ? platform.doc.createComment(`org-loc`)
...
const attachToAncestor = (hostRef, ancestorComponent) => {
    if (appData.BUILD.asyncLoading && ancestorComponent && !hostRef.$onRenderResolve$) {
...
const scheduleUpdate = (hostRef, isInitialLoad) => {
    ...
    if (appData.BUILD.asyncLoading && hostRef.$flags$ & 4 /* isWaitingForChildren */) {
    ...
    return then(promise, appData.BUILD.taskQueue
...
const updateComponent = (hostRef, instance, isInitialLoad) => {
    ...
    if (appData.BUILD.updatable && appData.BUILD.taskQueue) {
        hostRef.$flags$ &= ~16 /* isQueuedForUpdate */;
    }
    ...
    if (appData.BUILD.updatable || appData.BUILD.lazyLoad) {
...
const callRender = (instance) => {
    try {
        ...
        instance = (appData.BUILD.allRenderFn) ? instance.render() : (instance.render && instance.render());
    }
    catch (e) {
        platform.consoleError(e);
    }
...
const postUpdateComponent = (hostRef) => {
    ...
    if (appData.BUILD.asyncLoading) {
        ...
        if (hostRef.$flags$ & 512 /* needsRerender */) {
            platform.nextTick(() => scheduleUpdate(hostRef, false));
        }
...
const appDidLoad = (who) => {
    ...
    if (appData.BUILD.cssAnnotations) {
        platform.doc.documentElement.classList.add(HYDRATED_CLASS);
    }
    if (!appData.BUILD.hydrateServerSide) {
    ...
    if (appData.BUILD.profile && performance.measure) {
        performance.measure(`[Stencil] ${appData.NAMESPACE} initial load (by ${who})`, 'st:app:start');
    }
};
...
const setValue = (ref, propName, newVal, cmpMeta) => {
    ...
    if ((!appData.BUILD.lazyLoad || !(flags & 8 /* isConstructingInstance */) || oldVal === undefined) && newVal !== oldVal) {
        ...
        if (!appData.BUILD.lazyLoad || instance) {
            ...
            if (appData.BUILD.watchCallback && cmpMeta.$watchers$ && flags & 128 /* isWatchReady */) {
            ...
            if (appData.BUILD.updatable && (flags & (2 /* hasRendered */ | 16 /* isQueuedForUpdate */)) === 2 /* hasRendered */) {
...
const proxyComponent = (Cstr, cmpMeta, flags) => {
    if (appData.BUILD.member && cmpMeta.$members$) {
        ...
        members.forEach(([memberName, [memberFlags]]) => {
            if ((appData.BUILD.prop || appData.BUILD.state) && ((memberFlags & 31 /* Prop */) ||
                ((!appData.BUILD.lazyLoad || flags & 2 /* proxyState */) &&
            ...
            else if (appData.BUILD.lazyLoad && appData.BUILD.method && (flags & 1 /* isElementConstructor */) && (memberFlags & 64 /* Method */)) {
        ...
        if (appData.BUILD.observeAttribute && (!appData.BUILD.lazyLoad || flags & 1 /* isElementConstructor */)) {
            ...
            Cstr.observedAttributes = members
                ...
                if (appData.BUILD.reflect && m[0] & 512 /* ReflectAttr */) {
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
    // initializeComponent
    if ((appData.BUILD.lazyLoad || appData.BUILD.hydrateServerSide || appData.BUILD.style) && (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        // we haven't initialized this element yet
        hostRef.$flags$ |= 32 /* hasInitializedComponent */;
        if (appData.BUILD.mode && hostRef.$modeName$ == null) {
            // initializeComponent
        ...
        const scopeId = appData.BUILD.mode ? getScopeId(cmpMeta.$tagName$, hostRef.$modeName$) : getScopeId(cmpMeta.$tagName$);
        if (appData.BUILD.style && !platform.styles.has(scopeId) && Cstr.style) {
            ...
            if (!appData.BUILD.hydrateServerSide && appData.BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await new Promise(function (resolve) { resolve(require('./shadow-css.js')); }).then(m => m.scopeCss(style, scopeId, false));
            }
    ...
    if (appData.BUILD.asyncLoading && ancestorComponent && ancestorComponent['s-rc']) {
...
const connectedCallback = (elm) => {
    if ((platform.plt.$flags$ & 1 /* isTmpDisconnected */) === 0) {
        ...
        if (appData.BUILD.hostListener && cmpMeta.$listeners$) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (appData.BUILD.hydrateClientSide) {
                ...
                if (hostId) {
                    if (appData.BUILD.shadowDom && platform.supportsShadowDom && cmpMeta.$flags$ & 1 /* shadowDomEncapsulation */) {
                        const scopeId = appData.BUILD.mode ? addStyle(elm.shadowRoot, cmpMeta, elm.getAttribute('s-mode')) : addStyle(elm.shadowRoot, cmpMeta);
            ...
            if (appData.BUILD.slotRelocation && !hostId) {
                ...
                if ((appData.BUILD.hydrateServerSide) ||
                    ((appData.BUILD.slot || appData.BUILD.shadowDom) && cmpMeta.$flags$ & (4 /* hasSlotRelocation */ | 8 /* needsShadowDomShim */))) {
            ...
            if (appData.BUILD.asyncLoading) {
                ...
                while ((ancestorComponent = (ancestorComponent.parentNode || ancestorComponent.host))) {
                    ...
                    if ((appData.BUILD.hydrateClientSide && ancestorComponent.nodeType === 1 /* ElementNode */ && ancestorComponent.hasAttribute('s-id')) ||
            ...
            if (appData.BUILD.prop && appData.BUILD.lazyLoad && !appData.BUILD.hydrateServerSide && cmpMeta.$members$) {
...
const setContentReference = (elm) => {
    ...
    // create a node to represent where the original
    // content was first placed, which is useful later on
    const crName = (appData.BUILD.isDebug)
        ? `content-ref:${elm.tagName}`
        : '';
    const contentRefElm = elm['s-cr'] = platform.doc.createComment(crName);
...
const proxyCustomElement = (Cstr, compactMeta) => {
    ...
    Object.assign(Cstr.prototype, {
        __registerHost() {
            platform.registerHost(this, cmpMeta);
        },
        connectedCallback() {
...
const forceModeUpdate = (elm) => {
    if (appData.BUILD.style && appData.BUILD.mode && !appData.BUILD.lazyLoad) {
        ...
        if (hostRef.$modeName$ !== mode) {
            ...
            const flags = cmpMeta.$flags$;
            if (style) {
                if (!platform.styles.has(scopeId)) {
                    registerStyle(scopeId, style, !!(flags & 1 /* shadowDomEncapsulation */));
                }
                hostRef.$modeName$ = mode;
                elm.classList.remove(oldScopeId + '-h', oldScopeId + '-s');
                attachStyles(hostRef);
                platform.forceUpdate(elm);
            }
        }
    }
};
...
const cloneNodeFix = (HostElementPrototype) => {
    ...
    HostElementPrototype.cloneNode = function (deep) {
        const srcNode = this;
        const isShadowDom = appData.BUILD.shadowDom ? srcNode.shadowRoot && platform.supportsShadowDom : false;
...
const bootstrapLazy = (lazyBundles, options = {}) => {
    ...
    if (appData.BUILD.hydrateClientSide && appData.BUILD.shadowDom) {
        const styles = platform.doc.querySelectorAll('style[s-id]');
    ...
    lazyBundles.forEach(lazyBundle => lazyBundle[1].forEach(compactMeta => {
        ...
        const HostElement = class extends HTMLElement {
            ...
            constructor(self) {
                ...
                super(self);
                self = this;
                platform.registerHost(self, cmpMeta);
                if (appData.BUILD.shadowDom && cmpMeta.$flags$ & 1 /* shadowDomEncapsulation */) {
                    ...
                    else if (!appData.BUILD.hydrateServerSide && !('shadowRoot' in self)) {
            ...
            connectedCallback() {
                ...
                if (isBootstrapping) {
                    ...
                    platform.plt.jmp(() => connectedCallback(this));
                }
            }
            disconnectedCallback() {
...
const getElement = (ref) => appData.BUILD.lazyLoad ? platform.getHostRef(ref).$hostElement$ : ref;
...
const createEvent = (ref, name, flags) => {
    ...
    return {
        emit: (detail) => {
            ...
            const ev = new (appData.BUILD.hydrateServerSide ? platform.win.CustomEvent : CustomEvent)(name, {
...
const getContext = (_elm, context) => {
    if (context in platform.Context) {
        ...
        return appData.BUILD.hydrateServerSide ? true : false;
    }
    else if (context === 'isClient') {
        return appData.BUILD.hydrateServerSide ? false : true;
    }
