...
<div class="col-sm-6 p-r-25 p-r-15-sr991 p-b-25">
  <div class="how2 how2-cl5 flex-sb-c m-b-35">
    ...
    <a href="{{url('categ = Life')}}" class="tab01-link f1-s-1 cl9 hov-cl10 trans-03">
      View all
  ...
  @if ($Life_one)
    <!-- Main Item post -->
    <div class="m-b-30">
      <a href="read = {{$Life_one->header}}" class="wrap-pic-w hov1 trans-03">
        <img class="size-a-51"  src="indexfolder/images/{{$Life_one->img}}" alt="IMG">
      </a>

      <div class="p-t-20">
        <h5 class="p-b-5">
          <a href="read = {{$Life_one->header}}" class="f1-m-3 cl2 hov-cl10 trans-03">
            {{$Life_one->header}}
          </a>
        </h5>

        <span class="cl8">
          <a href="{{$Life_one->cat}} = {{$Life_one->type}}" class="f1-s-4 cl8 hov-cl10 trans-03">
          {{$Life_one->type}}
          </a>
          ...
          <span class="f1-s-3">
            {{ $Life_one->created_at->format('M d') }}
          </span>
        </span>
      </div>
    </div>

    @foreach ( $Life_Nav->take(3)   as $posts)

  @if ($posts->id != $Life_one->id)
