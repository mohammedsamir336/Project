...
<div class="container">
      ...
      <section class="my-5">
        ...
        <div class="row">
          ...
          <div class="col-lg-8">
            ...
            <form enctype="multipart/form-data" action="{{route('admin.uploadPost')}}" method="post">
                     @csrf
            <!--  card -->
            <div class="card mb-4 post-title-panel">
              <div class="card-body">
                <div class="md-form mt-1 mb-0">
                  <input type="text" id="header" name="header" class="form-control " value="{{$post->header}}" required>
        ...
        <input  id="test_header" type="hidden" name="headerTest" value="{{$post->header}}">
            ...
            <div class="card mb-4 post-title-panel">
              <div class="card-body">
                <div class="md-form mt-1 mb-0">
                  <input type="text" name="author" id="form2" class="form-control " value="{{$post->author}}" required>
            ...
            <div class="card mb-4 post-title-panel">
              <div class="card-body">
                <div class="md-form mt-1 mb-0">
                  <input type="text" name="tag" id="form3" class="form-control" value="{{$post->tag}}" required>
           ...
           <textarea id="mytextarea"  name="p1" required>{{$post->p1}}</textarea>
            </div>
            <!--  card -->
            <div class="card mb-4">
            <span class="ml-2">p2</span>
          <textarea   name="p2" >{{$post->p2}}</textarea>
            </div>

            <!--  card -->
            <div class="card mb-4">
              <span class="ml-2">p3</span>
          <textarea  name="p3" >{{$post->p3}}</textarea>
            </div>
            ...
            <div class="card mb-4 post-title-panel">
              <div class="card-body">
                <div class="md-form mt-1 mb-0">
                  <input type="url" name="video" id="form4" class="form-control" value="{{$post->videos_id()->first()->url}}"required>
                  <label class="form-check-label active" for="form4">Url video</label>
                  <input type="hidden" name="video_id" value="{{$post->videos_id()->first()->id}}">
                  {!!$post->videos_id()->first()->video ?? '' !!}
                </div>
              </div>
            </div>
            <!--  card -->
            ...
            <div class="card mb-4 post-title-panel">
              <div class="card-body">
                <div class="md-form mt-1 mb-0">
                  <input type="text" name="title" id="title" class="form-control" value="{{$post->videos_id()->first()->title}}"required>
      ...
      <input  id="test_title" type="hidden" name="titleTest" value="{{$post->videos_id()->first()->title}}">
              </div>
            </div>
            <!--  card -->
        @endif
              ...
              <!-- append Card  -->
                  <div id="type" class="card-body card-body-cascade" >
                    ...
                    <fieldset class="form-check mb-4" id="typefield"  >
                      <input  name="type" class="type form-check-input"  value="{{$post->type}}" checked  type="checkbox" id="type1">
                      <label class="form-check-label" for="type1" onclick="m();">{{$post->type}}</label>
                    </fieldset>
                ...
                <div class="card card-cascade narrower">
                  ...
                  <div class="card-body card-body-cascade">
                    ...
                    <input name="cat" class="check form-check-input " value="{{$post->cat}}" type="checkbox" checked="checked" id="cat-1">
                    <label  id="cat" class=" form-check-label" for="cat-1" onclick="e();$('#{{$post->cat}}').show();$('#type').hide(); ">{{$post->cat}}</label>
                    </fieldset>
      ...
      <div class="mt-4">
            ...
            <div class="container" style=" position:relative;bottom:100px;">
              <img  id="post_img" src="{{asset('indexfolder/images/'.$post->img)}}" class="hoverable" height="230" width="250" />
...
/* check unique post header by Ajax */
    ...
    $(document).ready(function(){
     ...
     $('#header').blur(function(){
      ...
      {
       $.ajax({
        url:"{{ route('admin.header_available.check') }}",
        method:"POST",
    ...
    /* check unique video title by Ajax */
        ...
        $(document).ready(function(){
         ...
         $('#title').blur(function(){
          ...
          {
           $.ajax({
            url:"{{ route('admin.title_available.check') }}",
            method:"POST",
